//
//  LogFaultReportingTests.swift
//
//  A run's log must not go missing quietly (#386). `FaultReportingTests`
//  covers the attachment half of the same post-condition.
//

import XCTest
@testable import XCTestHTMLReportCore

final class LogFaultReportingTests: XCTestCase {
    ...
    private func syntheticSummary(
        ...
        Summary(
            parsedRuns: (0 ..< runCount).map { _ in
                ParsedRun(
    ...
    func testTwoRunsLosingTheSameLogAreRecordedOnce() {
        // Passing one bundle twice produces two runs whose fault details are
        // identical, since the detail names the destination. Two entries the
        // reader cannot tell apart is noise, so dedup applies within a single
        // pass and not only across repeated calls.
        let collector = FaultCollector()
        let summary = syntheticSummary(
            logReference: "no-such-log", runCount: 2, collector: collector
        )
        ...
        XCTAssertEqual(
            summary.faults.map(\.kind), [.unresolvedLog],
            "One indistinguishable degradation is one fault"
        )
    }
